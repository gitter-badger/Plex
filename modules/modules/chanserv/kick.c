...
command_t cs_kick = { "KICK", N_("Kicks a user from a channel."),
                        AC_NONE, 3, cs_cmd_kick, { .path = "cservice/kick" } };
command_t cs_kickban = { "KICKBAN", N_("Kicks and bans a user from a channel."),
...
static void cs_cmd_kickban(sourceinfo_t *si, int parc, char *parv[])
{
	...
	if (is_service(tu))
	{
		command_fail(si, fault_noprivs, _("\2%s\2 is a service. You can't kick or deop themm"), tu->nick);
		return;
	}
