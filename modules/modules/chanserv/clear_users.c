...
static void cs_cmd_clear_users(sourceinfo_t *si, int parc, char *parv[])
{
	...
	if (parc >= 2)
		snprintf(fullreason, sizeof fullreason, "Users cleared by %s: %s", get_source_name(si), parv[1]);
	else
		snprintf(fullreason, sizeof fullreason, "Users cleared by %s", get_source_name(si));
